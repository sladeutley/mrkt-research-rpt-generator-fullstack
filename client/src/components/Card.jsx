...
import { downloadImage, downloadPDF } from '../utils';
...
const Card = ({ _id, name, prompt, industry, productsAndServices, keyMarketSegments, competitiveAnalysis }) => (
  <div className="relative w-full h-64 rounded-lg shadow-lg overflow-hidden bg-blue-500">
    ...
    <div className="absolute inset-x-0 bottom-0 h-16 bg-gray-900 opacity-0 transition-opacity duration-500 flex items-center justify-between px-4">
      ...
      <button type="button" onClick={() => downloadPDF(_id, prompt, industry, productsAndServices, keyMarketSegments, competitiveAnalysis)} className="outline-none bg-transparent border-none">
        <img src={download} alt="download" className="w-6 h-6 object-contain invert" />
      </button>
      {/* <button type="button" onClick={() => downloadPDF(_id)} className="outline-none bg-transparent border-none">
        <img src={download} alt="download" className="w-6 h-6 object-contain invert" />
      </button> */}
    </div>
  </div>
